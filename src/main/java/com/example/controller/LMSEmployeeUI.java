...
public class LMSEmployeeUI {
    ...
    public static void main(String[] args) throws IOException {
        // Start HTTP server on port 8080 (Render expects this)
        HttpServer server = HttpServer.create(new InetSocketAddress(8080), 0);
        server.createContext("/", new RootHandler());
        server.createContext("/apply", new ApplyHandler());
        server.setExecutor(null);
        server.start();
        System.out.println("Leave Management System is running on http://localhost:8080");
    }

    static class RootHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            // Build a simple HTML page with the dashboard and forms
            ...
            // Show leave balances for employee 1
    ...
    static class ApplyHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            if ("POST".equals(exchange.getRequestMethod())) {
                ...
                // Very simple parsing (in real code use a library)
                String type = extractParam(query, "type");
                String startStr = extractParam(query, "start");
                String endStr = extractParam(query, "end");
                String reason = extractParam(query, "reason");
